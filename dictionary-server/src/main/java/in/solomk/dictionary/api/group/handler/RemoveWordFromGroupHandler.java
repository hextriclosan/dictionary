...
@Component
@AllArgsConstructor
public class RemoveWordFromGroupHandler implements HandlerFunction<ServerResponse> {
    ...
    public Mono<ServerResponse> handle(ServerRequest request) {
        return request.principal()
                      ...
                      .flatMap(userId -> deleteWordFromGroupAndValidateStudiedLanguage(request, userId))
                      .then(Mono.defer(() -> ServerResponse.ok().build()));
    }

    private Mono<Void> deleteWordFromGroupAndValidateStudiedLanguage(ServerRequest request, String userId) {
        ...
        return userLanguagesService.validateLanguageIsStudied(userId, supportedLanguage)
                                   .then(deleteWordFromGroup(request, userId));
    }

    private Mono<Void> deleteWordFromGroup(ServerRequest request, String userId) {
        var supportedLanguage = extractLanguageCode(request);
        var groupId = request.pathVariable("groupId");
        var wordId = request.pathVariable("wordId");
        return wordsGroupService.deleteWordFromGroup(userId, supportedLanguage, groupId, wordId);
    }
