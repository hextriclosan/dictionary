...
@Component
@AllArgsConstructor
public class GetLearningItemHandler implements HandlerFunction<ServerResponse> {
    ...
    public Mono<ServerResponse> handle(ServerRequest request) {
        return request.principal()
                      ...
                      .flatMap(userId -> learningItemsService.getLearningItem(userId, extractLanguageCode(request), request.pathVariable("learningItemId")))
                      .map(mapper::toLearningItemResponse)
                      .flatMap(learningItemResponse -> ServerResponse.ok()
                                                             .contentType(MediaType.APPLICATION_JSON)
                                                             .bodyValue(learningItemResponse))
